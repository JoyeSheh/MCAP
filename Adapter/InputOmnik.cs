...
namespace Adapter
{
    ...
    internal class InputOmnik:IInputAdapter
    {
        ...
        private byte[] CreateRequestArray(byte control,byte function,byte[] data)
        {
            ...
            ushort checksum = 0;
            for(int i = 0;i<head.Length+data.Length;++i)
            {
                checksum+=result[i];
            }
        ...
        private bool IsValid(byte[] gram)
        {
            ...
            if(0x3a!=gram[0]||0x3a!=gram[1]||Inverter_Address!=gram[3]||AP_Address!=gram[4])
            {
                return false;
            }
        ...
        {
            if(!(isRegisted||TrySend(CreateRegisterArray())))
            {
                return false;
            }

            if(!(eventSlim.Wait(timeOut)&&isRegisted))
            {
                return false;
            }

            if(!TrySend(CreateRequestArray(0x11,0x10,new byte[0])))
            {
                return false;
            }
